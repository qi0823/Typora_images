...
us101_1 = "E:\ps\US-101-LosAngeles-CA\us-101-vehicle-trajectory-data\vehicle-trajectory-data\0750am-0805am\trajectories-0750am-0805am.txt";
us101_2 = "E:\ps\US-101-LosAngeles-CA\us-101-vehicle-trajectory-data\vehicle-trajectory-data\0805am-0820am\trajectories-0805am-0820am.txt";
us101_3 = "E:\ps\US-101-LosAngeles-CA\us-101-vehicle-trajectory-data\vehicle-trajectory-data\0820am-0835am\trajectories-0820am-0835am.txt";
i80_1 = "E:\ps\I-80-Emeryville-CA\i-80-vehicle-trajectory-data\vehicle-trajectory-data\0400pm-0415pm\trajectories-0400-0415.txt";
i80_2 = "E:\ps\I-80-Emeryville-CA\i-80-vehicle-trajectory-data\vehicle-trajectory-data\0500pm-0515pm\trajectories-0500-0515.txt";
i80_3 = "E:\ps\I-80-Emeryville-CA\i-80-vehicle-trajectory-data\vehicle-trajectory-data\0515pm-0530pm\trajectories-0515-0530.txt";
...
for ii = 1:6
    ...
    for k = 1:length(traj{ii}(:, 1))        
        ...
        % 获取速度和加速度
        traj{ii}(k, 9) = vehtraj(ind, 7); % 保存速度
        traj{ii}(k, 10) = vehtraj(ind, 8); % 保存加速度
        traj{ii}(k, 11) = vehtraj(ind, 9); % 保存车辆类别
        ...
        if ~isempty(frameL)
            for l = 1:size(frameL, 1)
                ...
                if abs(y) < 90
                    ...
                    traj{ii}(k, 11 + gridInd) = frameL(l, 2);  % 修改索引以适应新特征位置
                end
            end
        end
        for l = 1:size(frameEgo, 1)
            ...
            if abs(y) < 90 && y ~= 0
                ...
                traj{ii}(k, 11 + gridInd) = frameEgo(l, 2);  % 修改索引以适应新特征位置
            end
        end
        if ~isempty(frameR)
            for l = 1:size(frameR, 1)
                ...
                if abs(y) < 90
                    ...
                    traj{ii}(k, 11 + gridInd) = frameR(l, 2);  % 修改索引以适应新特征位置
                end
            end
        end
    end
...
for k = 1:size(trajTr, 1)
    t = trajTr(k, 3);
    if size(tracksTr{trajTr(k, 1), trajTr(k, 2)}, 2) >= 31 && tracksTr{trajTr(k, 1), trajTr(k, 2)}(1, 31) <= t && tracksTr{trajTr(k, 1), trajTr(k, 2)}(1, end) > t + 1
        indsTr(k) = 1;
    end
end
...
for k = 1:size(trajVal, 1)
    t = trajVal(k, 3);
    if size(tracksVal{trajVal(k, 1), trajVal(k, 2)}, 2) >= 31 && tracksVal{trajVal(k, 1), trajVal(k, 2)}(1, 31) <= t && tracksVal{trajVal(k, 1), trajVal(k, 2)}(1, end) > t + 1
        indsVal(k) = 1;
    end
end
...
for k = 1:size(trajTs, 1)
    t = trajTs(k, 3);
    if size(tracksTs{trajTs(k, 1), trajTs(k, 2)}, 2) >= 31 && tracksTs{trajTs(k, 1), trajTs(k, 2)}(1, 31) <= t && tracksTs{trajTs(k, 1), trajTs(k, 2)}(1, end) > t + 1
        indsTs(k) = 1;
    end
end
...
traj = trajTr;
tracks = tracksTr;
save('TrainSet1', 'traj', 'tracks');

traj = trajVal;
tracks = tracksVal;
save('ValSet1', 'traj', 'tracks');

traj = trajTs;
tracks = tracksTs;
save('TestSet1', 'traj', 'tracks');
