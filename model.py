...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        query = self.query(x).view(batch_size, seq_length, self.num_heads, input_dim)
        key = self.key(x).view(batch_size, seq_length, self.num_heads, input_dim)
        value = self.value(x).view(batch_size, seq_length, self.num_heads, input_dim)

        scores = torch.einsum('bqhd,bkhd->bhqk', [query, key]) / (input_dim ** 0.5)
        attn_weights = torch.nn.functional.softmax(scores, dim=-1)
        self.attn_weights = attn_weights
        attn_output = torch.einsum('bhqk,bkhd->bqhd', [attn_weights, value]).contiguous()

        attn_output = attn_output.view(batch_size, seq_length, self.num_heads * input_dim)
...
class highwayNet(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.ip_emb = torch.nn.Linear(5, self.input_embedding_size)  # 改为5以包含速度、加速度和类别
        ...
        self.multihead_attention = MultiHeadAttention(self.soc_embedding_size, num_heads=3)
        ...
        self.attn_weights = self.multihead_attention.attn_weights
    ...
    def forward(self, hist, nbrs, masks, lat_enc, lon_enc):
        ...
        attn_output = self.multihead_attention(soc_enc.unsqueeze(1)).squeeze(1)
        self.attn_weights = self.multihead_attention.attn_weights

        enc = torch.cat((attn_output, hist_enc), 1)
